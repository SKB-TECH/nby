        ...
        <section id="don-form" className="relative overflow-hidden bg-[#f4f6fa] px-5 py-24 sm:px-8">
            ...
            <div className="relative mx-auto grid max-w-[1160px] items-start gap-8 lg:grid-cols-[.72fr_1.28fr]">
                ...
                <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-[28px] border border-white bg-white/95 p-5 shadow-[0_30px_90px_rgba(7,17,23,.1)] backdrop-blur sm:p-10">
                    ...
                    <div className="mt-9"><FieldLabel>{copy.contactLabel}</FieldLabel><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={name} onChange={e => { setName(e.target.value); setReady(false); }} placeholder={copy.namePlaceholder} className="h-[58px] rounded-2xl border border-slate-200 px-5 text-sm outline-none transition focus:border-[#df9200]" /><input type="tel" value={phone} onChange={e => { setPhone(e.target.value); setReady(false); }} placeholder={copy.phonePlaceholder} className="h-[58px] rounded-2xl border border-slate-200 px-5 text-sm outline-none transition focus:border-[#df9200]" /></div></div>
                    </>}
                    ...
                    <button type="button" disabled={!formIsValid || submitting || !name.trim() || !phone.trim()} onClick={() => void submitContribution()} className="group mt-10 flex h-16 w-full items-center justify-between rounded-2xl bg-[#df9200] px-6 text-xs font-black uppercase tracking-wider text-white shadow-[0_15px_35px_rgba(223,146,0,.25)] transition hover:bg-[#071117] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none"><span className="flex items-center gap-3">{donationType === 0 ? <LockKeyhole className="h-4 w-4" /> : <HandHeart className="h-4 w-4" />}{submitting ? copy.submitting : donationType === 0 ? copy.continue : donationType === 1 ? copy.submitInKind : copy.submitSkill}</span><ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" /></button>
