...
export default function AppointmentBooking({ copy }: { copy: ContactCopy }) {
    ...
    useEffect(() => {
        publicApi<PublicAppointmentSlot[]>("/public/appointment-slots")
            .then(setSlots)
            .catch(fetchError => setError(fetchError instanceof Error ? fetchError.message : "Impossible de charger les créneaux."))
            .finally(() => setLoadingSlots(false));
    }, []);
    const slotsByDate = useMemo(() => slots.reduce<Record<string, PublicAppointmentSlot[]>>((groups, slot) => {
    ...
    const selectedDateLabel = date ? new Intl.DateTimeFormat(locale, { dateStyle: "long", timeZone: "Africa/Kinshasa" }).format(new Date(`${date}T12:00:00+02:00`)) : "";
    ...
    if (confirmed) {
        return <section className="px-5 py-24 sm:px-8"><motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} className="mx-auto max-w-2xl rounded bg-white p-10 text-center shadow-xl"><span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-[#fff0cf] text-[#df9200]"><Check className="h-9 w-9" /></span><h2 className="mt-7 font-serif text-4xl">{copy.confirmed}</h2><p className="mt-5 leading-8 text-slate-600">{copy.confirmedText}</p><div className="mx-auto mt-7 max-w-md rounded bg-[#071117] p-6 text-left text-white"><p className="font-bold">{type}</p><p className="mt-2 text-sm text-white/65">{selectedDateLabel} · {selectedSlot ? new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit", timeZone: "Africa/Kinshasa" }).format(new Date(selectedSlot.startsAt)) : ""}</p></div><button type="button" onClick={reset} className="mt-7 bg-[#df9200] px-7 py-4 text-xs font-bold uppercase tracking-wider text-white">{copy.another}</button></motion.div></section>;
    }
            ...
            <form onSubmit={submit} className="mx-auto grid max-w-[1180px] items-start gap-8 lg:grid-cols-[1fr_360px]">
                <div className="space-y-8">
                    ...
                    <Step title={copy.chooseDate} icon={CalendarDays}>
                        {loadingSlots ? <p className="text-sm text-slate-400">Chargement des disponibilités…</p> : <AppointmentCalendar value={date} locale={locale} availableDates={availableDates} onChange={(selectedDate) => { setDate(selectedDate); setTime(""); }} />}
                    </Step>
                    <Step title={copy.chooseTime} icon={Clock3}>
                        {!date ? <p className="text-sm text-slate-400">{copy.chooseDate}</p> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{(slotsByDate[date] ?? []).map(slot => <ChoiceButton key={slot.id} active={time === slot.id} onClick={() => setTime(slot.id)}>{new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit", timeZone: "Africa/Kinshasa" }).format(new Date(slot.startsAt))}</ChoiceButton>)}</div>}
