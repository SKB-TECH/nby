...
type SermonItem = { id: string; title: string; category: string; date: string; thumbnailUrl?: string };

export default function SermonsGrid({ copy, sermonsData }: { copy: MessagesCopy; sermonsData?: SermonItem[] }) {
    const sermonItems: SermonItem[] = sermonsData ?? sermons;
                        ...
                        <motion.article key={sermon.id} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -7 }} viewport={{ once: true }} transition={{ delay: index * .06 }} className="group">
                            <button type="button" onClick={() => setSelected(sermon)} className="w-full text-left">
                                <div className="relative aspect-video overflow-hidden bg-[#071117]">
                                    <img src={sermon.thumbnailUrl || `https://i.ytimg.com/vi/${sermon.id}/hqdefault.jpg`} alt={sermon.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-110" />
